...
import cn.szu.blankxiao.mycalendar.util.yearMonth
 ...
 * @param outDateStyle 月份外日期样式 固定每月六周/根据实际每月5周或六周
...
/**
 * 支持月/周模式切换的自定义日历状态 含有LazyListState以控制HorizontalPager
 */
@Stable
class CustomCalendarState internal constructor(
	...
	fun onPageChanged(newPageIndex: Int) {
		...
		val newDate = when (_calendarMode) {
			...
			CalendarMode.WEEK -> {
				// 周模式：计算新周的对应日期，保持同一星期数(本周周一切换后是下周周一)
				val weekStartDate = CalendarDataCalculator.getDateByWeekIndex(
	...
	suspend fun finishDragAndSnap(velocity: Float = 0f) {
		// 考虑速度：快速滑动可以更容易切换
		val threshold = if (abs(velocity) > 1000f) {
			0.3f
		} else {
			0.5f
		}
